...
public class HC_DVRManager {
	...
	protected static int dwSpeed = 2;
	...
	public void loginDevice() {
		...
		m_userID = HCNetSDK.getInstance().NET_DVR_Login_V30(ip, port, username,
		...
		try {
			...
			if (m_iPlayID < 0) {
				...
				m_iPlayID = HCNetSDK.getInstance().NET_DVR_RealPlay_V30(
						m_userID, ClientInfo, getRealPlayerCallBack(), true);
	...
	public void changePlayChannl(DeviceBean bean) {
		if ((!ip.equals(bean.getIP()))
			...
			originalBean.setChannel(String.valueOf(channel));
			...
			realPlay();
		} else if (channel != Integer.parseInt(bean.getChannel())) {
	...
	public void logoutDevice() {
		if (HCNetSDK.getInstance().NET_DVR_Logout_V30(m_userID)) {
			m_userID = -1;
			Log.i(TAG, "登出设备成功！");
		} else {
			m_userID = 0;
		...
		try {
			...
			case HCNetSDK.NET_DVR_SYSHEAD:
				...
				if (iDataSize > 0) {
					...
					if (Player.getInstance().setDisplayBuf(m_iPort, 15)) { // 帧率，不设置为默认15
	...
	public void startFocus(int x) {
		...
		if (x < 0) {
			HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed(m_iPlayID,
					PTZCommand.FOCUS_NEAR, 0, dwSpeed);
		} else {
			HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed(m_iPlayID,
					PTZCommand.FOCUS_FAR, 0, dwSpeed);
		}
	}
	...
	public void stopFocus(int x) {
		...
		if (x < 0) {
			HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed(m_iPlayID,
					PTZCommand.FOCUS_NEAR, 1, dwSpeed);
		} else {
			HCNetSDK.getInstance().NET_DVR_PTZControlWithSpeed(m_iPlayID,
					PTZCommand.FOCUS_FAR, 1, dwSpeed);
		}
	}
	 ...
	 * @return
	 */
	public boolean sreenShort(String name) {
		NET_DVR_JPEGPARA dvr_JPEGPARA = new NET_DVR_JPEGPARA();
		// 图片质量系数：0-最好，1-较好，2-一般
		dvr_JPEGPARA.wPicQuality = 0;
		dvr_JPEGPARA.wPicSize = 0xff;
		if (HCNetSDK.getInstance().NET_DVR_CaptureJPEGPicture(m_userID,
				channel + deviceInfo_V30.byStartChan, dvr_JPEGPARA,
				SmortHomeUtils.getPictrueDir() + name)) {
			return true;
		} else {
			Log.e(TAG,
					"截图失败！"
	...
	public boolean saveVideo(String name) {
		if (HCNetSDK.getInstance().NET_DVR_SaveRealData(m_userID,
				SmortHomeUtils.getVideoDir() + name)) {
			return true;
		} else {
			Log.e(TAG,
					"录视频失败！"
	...
	public boolean stopSaveVideo() {
		if (HCNetSDK.getInstance().NET_DVR_StopSaveRealData(m_userID)) {
			return true;
		} else {
			Log.e(TAG,
					"录视频失败！"
